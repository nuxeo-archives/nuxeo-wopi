...
public class TestFilesEndpoint {
    ...
    public void testGetFile() throws IOException {
        ...
        try (CloseableClientResponse response = get(joeToken, hugeBlobDoc.getId(), CONTENTS_PATH)) {
        ...
        try (CloseableClientResponse response = get(joeToken, headers, blobDoc.getId(), CONTENTS_PATH)) {
            assertEquals(412, response.getStatus());
        }
        ...
        // success - no header
        try (CloseableClientResponse response = get(joeToken, blobDoc.getId(), CONTENTS_PATH)) {
            assertEquals(200, response.getStatus());
            Blob actualBlob = Blobs.createBlob(response.getEntityInputStream());
            assertEquals(expectedBlob.getString(), actualBlob.getString());
        }
    ...
    public void testPutFile() throws IOException {
        ...
        // fail - 409 - joe has no write permission
        try (CloseableClientResponse response = post(joeToken, data, headers, zeroLengthBlobDoc.getId(),
                CONTENTS_PATH)) {
            assertEquals(409, response.getStatus());
        }

        // success - 200 - blob updated
        try (CloseableClientResponse response = post(johnToken, data, headers, zeroLengthBlobDoc.getId(),
                CONTENTS_PATH)) {
            assertEquals(200, response.getStatus());
            String itemVersion = response.getHeaders().getFirst(ITEM_VERSION);
        ...
        try (CloseableClientResponse response = post(johnToken, data, headers, blobDoc.getId(), CONTENTS_PATH)) {
            assertEquals(409, response.getStatus());
            String lock = response.getHeaders().getFirst(LOCK);
            assertEquals("", lock);
        }
        ...
        // fail - 409 - joe has no write permission
        headers.put(OVERRIDE, Operation.PUT.name());
        try (CloseableClientResponse response = post(joeToken, data, headers, blobDoc.getId(), CONTENTS_PATH)) {
            assertEquals(409, response.getStatus());
        }
        ...
        session.getDocument(hugeBlobDoc.getRef()).setLock();
        transactionalFeature.nextTransaction();
        try (CloseableClientResponse response = post(johnToken, data, headers, hugeBlobDoc.getId(), CONTENTS_PATH)) {
            assertEquals(409, response.getStatus());
            assertTrue(session.getDocument(hugeBlobDoc.getRef()).isLocked());
        }
    }
    ...
    public void testPutRelativeFile() throws IOException {
        ...
        // fail - 501 - no headers
        try (CloseableClientResponse response = post(johnToken, data, headers, blobDoc.getId())) {
        ...
        try (CloseableClientResponse response = post(joeToken, data, headers, blobDoc.getId())) {
            assertEquals(501, response.getStatus());
        }
    ...
    public void testGetShareUrl() throws IOException {
        ...
        try (CloseableClientResponse response = post(johnToken, headers, blobDoc.getId())) {
            ...
            assertEquals("http://localhost:18090/wopi/view/test/" + blobDoc.getId(),
                    jsonNode.get(SHARE_URL).textValue());
        }
        ...
        try (CloseableClientResponse response = post(johnToken, headers, blobDoc.getId())) {
            assertEquals(200, response.getStatus());
            JsonNode jsonNode = mapper.readTree(response.getEntityInputStream());
            assertEquals("http://localhost:18090/wopi/edit/test/" + blobDoc.getId(),
                    jsonNode.get(SHARE_URL).textValue());
        }
    }
